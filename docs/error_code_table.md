...
### 1. 参数错误 (1xxxx)
*主要对应目前的 HTTP 400 状态码，表示客户端传来的数据有问题。*

| 错误码 | 英文枚举 (枚举名) | 中文描述 | 对应现有代码场景 |
| :--- | :--- | :--- | :--- |
...
### 2. 资源不存在 (2xxxx)
*主要对应目前的 HTTP 404 状态码，表示客户端请求的数据在数据库里找不到。*

| 错误码 | 英文枚举 (枚举名) | 中文描述 | 对应现有代码场景 |
| :--- | :--- | :--- | :--- |
| **20101** | `PRODUCT_NOT_FOUND` | 商品不存在 | `product_controller.cc` 捕获到 `UnexpectedRows`，或 `Product not found.` |
| **20201** | `ORDER_NOT_FOUND` | 订单不存在 | （预留）未来查询订单详情时找不到 |

---

### 3. 业务错误 (3xxxx)
*主要对应目前的 HTTP 409 或 400 状态码，参数虽然合法，但违反了系统的核心业务规则。*
...
| **30302** | `PRODUCT_NOT_AVAILABLE`| 商品已下架 | （预留）未来基于 `available_listing` 字段拦截下单 |

---

### 4. 数据库与系统错误 (4xxxx)
*主要对应目前的 HTTP 500 状态码，表示服务器内部崩溃或数据库挂了。*
